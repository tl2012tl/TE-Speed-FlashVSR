...
from ...te_runtime.attention import (
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask=None, return_KV=False):
    if attention_mask is not None:
        x = sparge_attention(q, k, v, attention_mask, num_heads)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, f=None, h=None, w=None, local_num=None, topk=None,
        ...
        if uses_split_k_mask(k_w):
            attention_mask = generate_sparge_mask(
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        topk_ratio = 2.0 if topk_ratio is None else float(topk_ratio)
