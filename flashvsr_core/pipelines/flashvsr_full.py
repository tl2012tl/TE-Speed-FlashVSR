...
""")
    ...
    def __call__(
        ...
        # In staged mode the VAE is not needed during DiT denoising.  Keeping
        # it offloaded here makes TE's memory_policy meaningful for Full:
        # staged/auto lowers the peak, while resident keeps both models ready
        # and avoids the transfer before decoding.
        self.load_models_to_device(["dit"] if stage_memory else ["dit", "vae"])
        ...
        with torch.no_grad():
            for cur_process_idx in progress_bar_cmd(range(process_total_num)):
                ...
                chunk_topk_ratio = topk_ratio
                chunk_kv_ratio = kv_ratio
                chunk_local_range = local_range
                if budget_controller is not None:
                    chunk_topk_ratio, chunk_kv_ratio, chunk_local_range = budget_controller.values_for(
                        cur_process_idx, topk_ratio, kv_ratio, local_range
                    )
                log_chunk = (
                    cur_process_idx == 0
                    or cur_process_idx + 1 == process_total_num
                    or (cur_process_idx + 1) % 4 == 0
                )
                if log_chunk:
                    print(
                        f"[FlashVSR] denoise chunk {cur_process_idx + 1}/{process_total_num} "
                        f"topk={chunk_topk_ratio:.3f} kv={chunk_kv_ratio:.1f} "
                        f"local={chunk_local_range}",
                        flush=True,
                    )
                ...
                noise_pred_posi, pre_cache_k, pre_cache_v = model_fn_wan_video(
                    ...
                    topk_ratio=chunk_topk_ratio,
                    kv_ratio=chunk_kv_ratio,
                    cur_process_idx=cur_process_idx,
                    t_mod=self.t_mod,
                    t=self.t,
                    local_range = chunk_local_range,
                )
            ...
            if stage_memory and hasattr(self, 'dit') and not next(self.dit.parameters()).is_cpu:
            ...
            # Color correction. Tiled Wan VAE decode returns CPU frames. Move
            # only a short temporal slice to the LQ tensor's device instead
            # of copying the complete Full output back to VRAM at once.
            if color_fix:
                try:
                    correction_device = LQ_video.device
                    if frames.device == correction_device:
                        ...
                        corrected_parts = []
                        for correction_start in range(0, frames.shape[2], 16):
                            correction_end = min(frames.shape[2], correction_start + 16)
                            corrected = self.ColorCorrector(
                                frames[:, :, correction_start:correction_end].to(correction_device),
                                LQ_video[:, :, correction_start:correction_end],
                                clip_range=(-1, 1),
                                chunk_size=None,
                                method='adain'
                            )
                            corrected_parts.append(corrected.to(frames.device))
                        frames = torch.cat(corrected_parts, dim=2)
                except Exception as error:
                    print(
                        f"[FlashVSR] Full color correction skipped: {error}",
                        flush=True,
                    )
